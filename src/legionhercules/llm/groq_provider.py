"""Groq API provider for LEGIONHERCULES - Ultra-fast inference."""
...
class GroqProvider(LLMProvider):
    ...
    Supports Llama 3, Mixtral, Gemma models with incredibly fast inference.
    Free tier available. Get API key from: https://console.groq.com/
    """
    ...
    def __init__(
        ...
        timeout: float = 60.0,  # Lower timeout due to fast inference
    ):
    ...
    async def initialize(self) -> None:
        ...
        if not self.api_key:
            raise ValueError("Groq API key required. Set GROQ_API_KEY environment variable. Get free key at https://console.groq.com/")
    ...
    async def chat(
        ...
        for msg in messages:
            ...
            groq_messages.append(chat_msg.to_openai_dict())
    ...
    def _convert_tool_schema(self, tool: Any) -> dict[str, Any]:
        """Convert tool to Groq format (OpenAI compatible)."""
